...
1. In 1-2 sentences, explain what React's `useRouteMatch` hook is used for.
    useRouteMatch is useful for anytime you would use route to get to its match data - use route match and you can match the url the same way route would, very useful outside of a switch.
2. How would you explain form validation to someone who has never programmed before?
    Form validation is important to make sure the right information is entered and sent off so that information can be handled correctly. You can imagine this as a real life scenario involving a visit to the doctors office where you show up 155 minutes early to fill out forms and then hand them in to an admissions nurse who reads them over, hands them back and says 'no no, this part is going to be your insurance client number, instead of insurance company name, we need a number not the name.' and you erase the name and put the number down and they accept it. 
3. In 1-2 sentences, define end to end testing.
    end to end testing not unlike form validation is going to validate code you have written or that has been written from one end(backend) to the other (client side) by checking each line...or rather block of code against thoughtful useful tests that are written to make sure that information is handled and rendered correctly.
